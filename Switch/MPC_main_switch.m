...
hum.pregoal = [2.5 1];
hum.actgoal = [2.5 1];
% Human intention state(whether see the robot and willing to cooperate)
hum.S = 20;
lo_notsee = 2;
lo_see = 1;
meas_threshold =1;
...
%     Method{iter}=1;
    ...
    for tspan= t_fine : t_fine : t_upd
        ...
        for tsint = 0: t_int : t_fine-t_int
            ...
            % Predict the human motion with the assumption that he doesn't see
            % the robot
            dx_hnotsee = system_model_HUMpre_reac(xh_notsee,hum);
            xh_notsee_new = xh_notsee' + dx_hnotsee * t_int;
            xh_notsee = xh_notsee_new';
            
            % Predict the human motion with the assumption that he sees
            % the robot
            [dx_hsee,~] = system_model_HUMpre_pro(xh_see,xr,Con(count,:),rob,hum);
            xh_see_new = xh_see' + dx_hsee * t_int;
            xh_see = xh_see_new';
...
%             [dx_h,~] = system_model_HUMact_pro(xh,xr,Con(count,:),rob,hum);
        ...
        % Evaluate the human intention, during each t_fine the robot can uodate
        % the evaluation of the human cooperation state
        % Robot make the mearsurement from human motion whether it is
        % closer to the result predicted of human don't see the robot or
        % closer to the result predicted of human see the robot
        
        % When robot is very far from the human, the human will not
        % influence the human behavior, the difference between see and not
        % see robot is not big, thus the mearsure of human state dose not
        % make sense
        msure_see = 0;
        % When the different between see and not see assumption beyond a
        % certain threshold, and the error below a certain threshold, the measurement make sense
        if norm(xh - xh_notsee) - norm(xh - xh_see) > diff_threshod && norm(xh-xh_see) < meas_threshold
            msure_see =1;
        end
        if norm(xh - xh_see) - norm(xh - xh_notsee) > diff_threshod && norm(xh-xh_notsee)<meas_threshold
            msure_see =2;
        end
        % Update the evaluation function of the human desire
        if msure_see ==1
            hum.S =  hum.S + lo_see;
        end
        if msure_see ==2
            hum.S = hum.S - lo_notsee;
        end
