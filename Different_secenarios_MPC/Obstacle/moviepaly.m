...
tgor = 0 : t_fine : TP;  % Time step for planning in horizon time
tgmov = 0 : stepsize : TP; % Time stap for playing predicted path movie
...
for tt=1:step:size(rob.movS,1)
    ...
    for j=1:num_obs
        viscircles(map_obs(j,1:2),map_obs(j,3),'Color','c','LineWidth',1);
        hold on
    end
...
%             rob.glomov{iter} = interp1(tgor,rob.iniX{iter},tgmov);
...
%             text(rob.planmov{iter}(1,1),rob.planmov{iter}(1,2),str,'Interpreter','Latex','fontsize',13);
        ...
        if Method == 2
            ...
            text(5.5,15,str,'Interpreter','Latex','fontsize',15);
            str = {'Energy consuming f2:' num2str(f2{iter})};
            text(5.5,13,str,'Interpreter','Latex','fontsize',15);
...
%             str = {'Distance to the wall f6:' num2str(f6{iter})};
%             text(5.5,5,str,'Interpreter','Latex','fontsize',15);
%             str = {'Distance to the obstacle f7:' num2str(f7{iter})};
%             text(5.5,3,str,'Interpreter','Latex','fontsize',15);
%             str = {'Distance to the human f8:' num2str(f8{iter})};
...
%         plot(rob.glomov{iter}(:,1),rob.glomov{iter}(:,2),'--*','LineWidth',1,'Color',color(iter,:));
        hold on
...
%         plot(rob.glomov{iter}(:,1),rob.glomov{iter}(:,2),'--*','LineWidth',1,'Color',color(iter,:));
    ...
    if Method==1
        ...
        title({['Reactive Planning'];cost;['Constraint: g1 g2 g4'];['HSFM Simulation']},'Interpreter','Latex','FontSize',15)
    end
    if Method==2
        cost = sprintf('%s%s%s%s%s%s%s%s%s%s%s','Objective function: ', num2str(w1), '*f1+' , num2str(w2) ,'*f2+', num2str(w3), '*f3+', num2str(w4*20), '*f4+', num2str(w5*20), '*f5');
        title({['Cooperative Planning'];cost;['Constraint: g1 g2 g3 g4 g5'];['EHSFM with explicit collision prediction']},'Interpreter','Latex','FontSize',15)
    end
    M=getframe(h);
    writeVideo(vid,M);
end
