...
%     text(rob.goal(1)-1, rob.goal(2)-0.2,str,'fontsize',13);
    ...
    str={'Predicted'};
    text(hum.pregoal(1)+0.5, hum.pregoal(2)-0.2,str,'fontsize',13);
    hold on
    ...
    if tt<size(rob.movS,1) - 1/t_fine-1
        for iter = 1: fix((tt - 1) * stepsize) + 1
            ...
            str = {num2str(iter-1)};
            text(rob.planmov{iter}(1,1),rob.planmov{iter}(1,2),str,'Interpreter','Latex','fontsize',15);
            text(hum.premov{iter}(1,1),hum.premov{iter}(1,2),str,'Interpreter','Latex','fontsize',15);
            hold on
...
%         hold on
    ...
    if Method{iter}==1
        ...
        title({['Reactive Planning'];cost;['HSFM Simulation']},'Interpreter','Latex','FontSize',15)
    end
    if Method{iter}==2
        ...
        title({['Proactive-Cooperative Planning'];cost;['Constraint: g1 g2 g3'];['EHSFM with explicit collision prediction']},'Interpreter','Latex','FontSize',15)
    end
    M=getframe(h);
    writeVideo(vid,M);
end
